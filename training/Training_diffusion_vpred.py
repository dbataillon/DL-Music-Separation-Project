...
# Log-magnitude transform constants
LOG_SCALE = 1000.0  # Scaling factor before log (controls dynamic range)
LOG_OFFSET = 1.0    # Offset to avoid log(0)


def linear_to_log_magnitude(x):
    """Convert linear magnitude spectrogram to log scale.
    
    Maps sparse [0, 1] spectrograms to a more uniform distribution.
    Formula: log1p(x * scale) / log1p(scale)
    """
    return torch.log1p(x * LOG_SCALE) / math.log1p(LOG_SCALE)


def log_to_linear_magnitude(x):
    """Convert log magnitude back to linear scale.
    
    Inverse of linear_to_log_magnitude.
    """
    return (torch.exp(x * math.log1p(LOG_SCALE)) - 1.0) / LOG_SCALE
...
def train_one_epoch(model, loader, optimizer, device, betas, alphas_bar, T, accum_steps=2):
    ...
    for batch_idx, (mix_mag, y_clean) in enumerate(loader):
        ...
        running_loss += loss.item() * accum_steps * bsz
        n_samples += bsz
...
def train_diffusion():
    ...
    # Training loop
    # Limit samples per epoch to avoid OOM - subsample if needed
    MAX_SAMPLES_PER_EPOCH = 8000  # ~4GB GPU memory with batch=8
    ...
    for epoch in range(start_epoch, EPOCH + 1):
        ...
        # Subsample to limit memory usage
        n_samples = X_np.shape[0]
        if n_samples > MAX_SAMPLES_PER_EPOCH:
            indices = np.random.choice(n_samples, MAX_SAMPLES_PER_EPOCH, replace=False)
            X_np = X_np[indices]
            Y_np = Y_np[indices]
            print(f"  Subsampled {n_samples} -> {MAX_SAMPLES_PER_EPOCH} patches")
        ...
        # Apply log-magnitude transform to spread out sparse data
        # This helps diffusion work better on spectrograms
        X_t = linear_to_log_magnitude(X_t)
        Y_t = linear_to_log_magnitude(Y_t)
